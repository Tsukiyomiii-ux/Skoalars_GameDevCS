...
func _ready() -> void:
	...
	var study_btn = get_node_or_null("Control/MarginContainer/VBoxContainer/Popup/open_cont/HBoxContainer/NinePatchRect/PanelContainer/MarginContainer/VBoxContainer/NinePatchRect2/study_btn")
	if study_btn:
...
func _set_mouse_filter_recursive(node: Node):
	if node is Control:
		# ✅ Only stop clicks on actual interactive elements
		if not node is Button and not node is TextureButton and not node is HSlider:
			node.mouse_filter = Control.MOUSE_FILTER_IGNORE  # Changed from PASS to IGNORE
