...
func _ready():
	...
	hint_bg_dim.hide() 
	rescue_video.hide() # Keep hidden until the rescue button is clicked
	
	# Connect Quiz Buttons
	for btn in choice_btns:
		if btn:
			if btn.pressed.is_connected(self._on_answer_selected):
				btn.pressed.disconnect(self._on_answer_selected)
	...
	rescue_button.pressed.connect(_on_rescue_pressed) # Connected to the new video logic
...
func _on_button_hover(btn):
	if not is_transitioning: btn.modulate = Color(0.4, 0.6, 1.0)

func _on_button_exit(btn):
	if not is_transitioning: btn.modulate = Color.WHITE
...
func _on_rescue_pressed():
	# Plays the video in full screen when called
	rescue_video.show()
	rescue_video.play()
	win_popup.hide()

func _on_video_finished():
	# 1. Set the destination back to the Zypheria map
	GameManager.next_scene_path = "res://Assets/Scene/zypheria.tscn"
	
	# 2. Use the CORRECT path to the loading screen
...
func _on_quit_pressed(): get_tree().change_scene_to_file("res://Assets/Scene/zypheria.tscn")
